...
//			System.out.println("rlist : " + rlist.size());
			for(Rest rest2 : rlist) {
//				System.out.println("rest2" + rlist.toString());		
			}	
		}catch(Exception e) {
			e.printStackTrace(); 
		}
...
//		System.out.println("소개정보조회 check : " + check);	
		...
		urlBuilder.append("&" + URLEncoder.encode("MobileApp","UTF-8") + "=" + URLEncoder.encode("AppTest", "UTF-8")); /*서비스명=어플명*/
		for(int i=0; i<1; i++) {
			urlBuilder.append("&" + URLEncoder.encode("contentId","UTF-8") + "=" + URLEncoder.encode(conList[i], "UTF-8")); /*콘텐츠 ID*/
		}
...
//				System.out.println(rlist.size());
				for(Rest rest2 : rlist) {
//					System.out.println("rest2" + rest2.toString());						
				}	
			}catch(Exception e) {
				e.printStackTrace(); 
			}
...
//	        urlBuilder.append("&" + URLEncoder.encode("numOfRows","UTF-8") + "=" + URLEncoder.encode("10", "UTF-8")); /*한 페이지 결과 수*/
			...
			try {
				...
				for(int i=0; i<con.getLength(); i++ ) {
					...
					int pMaxCount = Integer.parseInt(roomCount.getFirstChild().getNodeValue());
					System.out.println("roomCount" + roCount);
					...
					System.out.println("roomOffSeasonMinFee1" + pMaxCount);
