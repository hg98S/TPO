...
public class Pagination {
	...
	// PageInfo 객체를 리턴해주는 메소드를 추가
		...
		// 그래서 그냥 메소드가 아닌 satic 메소드로 만들어줌
		...
		public static PageInfo getPageInfo(int currentPage, int listCount) {
			...
			int pageLimit = 10; //  한페이지에서 보여줄 네비게이션 수
			...
			// 전체 게시글 갯수 123 한페이지당 5개씩
			// 총 페이지 수는 25페이지
			// 0.1 일때 int 로 형변환하면 나머지 게시글이 잘리기 때문에
			// 0.9 더해서 더 크게 만들어줌
			 maxPage = (int)((double) listCount/boardLimit + 0.9);
			 
			 // 현재 페이지에서 시작하는 첫번째 페이지 값
			 startPage = (((int)((double) listCount/boardLimit + 0.9))-1)*pageLimit +1;
